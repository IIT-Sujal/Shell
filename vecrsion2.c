...
int executeCommand(char* cmd,vector* historyOfCommands,vector* allProcess)
{
	...
	else if(first_char(argc)=='!')
	{
		...
		{
			if(prefix_match(argc,vector_get(historyOfCommands,i)))
			{
				print_command(vector_get(historyOfCommands,i));
				executeCommand(vector_get(historyOfCommands,i),historyOfCommands,allProcess);
				break;
			}
			else if(i==0)
			{
	...
	else if(argc[0]=='#')
	{
		...
		if(n==1 && (int)vector_size(historyOfCommands)>taskID)
		{
			...
			executeCommand(cmdLine,historyOfCommands,allProcess);
		}
		else
		{
			print_invalid_index();
...
void free_main_memory(vector* historyOfCommands, vector* listOfCommands,char* historyFile,vector* allProcess)
{
	if(historyFile!=NULL)
	{
...
int shell(int argc, char *argv[]) {
	...
	if(vector_size(listOfCommands)>0)
	{
		...
		free_main_memory(historyOfCommands,listOfCommands,historyFile,allProcess);
		return 0;
	}
	...
	while (1)
	{
		...
		else if(x>0)
		{
			if(x==1)
			{
				...
				int status=executeCommand(cmd1,historyOfCommands,allProcess);
				if(status==0)
				{
					executeCommand(cmd2,historyOfCommands,allProcess);
				}
				free(cmd1);
				free(cmd2);
			}
